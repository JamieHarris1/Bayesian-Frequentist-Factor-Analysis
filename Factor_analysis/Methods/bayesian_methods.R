...
run_gibbs <- function(X, nrun, thin, k_tilde_0, save_folder, seed1, seed2){
  set.seed(seed1)
  gibbs(data=X, k_tilde=k_tilde_0, nrun=nrun, chain=1, thin=thin,
        save_folder=save_folder)
  
  print("CHAIN 2")
  
  set.seed(seed2)
  gibbs(data=X, k_tilde=k_tilde_0, nrun=nrun, chain=2, thin=thin,
        save_folder=save_folder)
...
assess_ks <- function(nrun, thin, save_folder){
  ...
  k_tilde_t <- matrix(NA, nrow=2, ncol=(nrun/thin))
  k_star_t <- matrix(NA, nrow=2, ncol=(nrun/thin))
  for(chain in 1:2){
    for(n in seq(from=thin, to=nrun, by=thin)){
      ...
      file_name <-paste0("Factor_analysis/Results/", save_folder,"/chain_",chain,"_iter_",
                         iter,".RData")
      load(file = file_name)
      k_tilde_t[chain,n/thin] <- k_tilde
      k_star_t[chain,n/thin] <- k_star
      
    }
  }
  #assess k_tilde
  plot(ts(k_tilde_t[1,]), ylab="k_tilde")
  lines(ts(k_tilde_t[2,]), col="red")
  
  #find optimal k_star
  tbl <- table(k_star_t)
  k_star <- as.numeric(names(tbl)[which.max(tbl)])
  plot(ts(k_star_t[1,]), ylab="k_star")
  lines(ts(k_star_t[2,]), col="red")
  return(list(k_star=k_star, k_star_t=k_star_t, k_tilde_t=k_tilde_t))
}

create_lambda_object <- function(nrun, thin, k_star_post, k_star_t, save_folder, n, p){
  # -- select lambda with dim k_star -- 
  n_samples <- min(rowSums(k_star_t==k_star))
